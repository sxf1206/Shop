<template name="zy-search">
	<view>
		<view class="search" style="background-color: #ff6600;padding-bottom: 8px;">
			<!-- #ifdef APP-PLUS -->
				<image src="/static/zy-search/voice.svg" mode="aspectFit" @click="startRecognize()" class="voice-icon" style="margin-top: 28px;margin-left: 16px;"></image>
			<!-- #endif -->
	...
	export default{
		...
		data() {
			return {
				...
				wantList:['网红面膜','补水保湿','遮瑕','祛痘']	//初始化推荐列表
			};
		},
		methods: {
			searchStart: function() {	//触发搜索
				...
				if (_this.searchText == '') {
					...
					uni.navigateTo({
						url: './SearchProducts?searchText='+_this.searchText
					});
				}
			},
...
<style lang="less" scoped>
	.search{
		...
		.closeview{
			position: absolute;right: 16px; top: 34px; z-index: 10;	
			/*  #ifdef  MP-WEIXIN  */
					top: 5px;
			/*  #endif  */
		}
