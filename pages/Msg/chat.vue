<template>
	<view>
			...
			<swiper class="swiper" indicator-dots="true" duration="150">
				<swiper-item v-for="(page,pid) in emojiList" :key="pid">
					<view v-for="(em,eid) in page" :key="eid" @tap="addEmoji(em)">
						<image mode="widthFix" :src="'https://www.chinaaos.com/static/img/emoji/'+em.url"></image>
					</view>
				</swiper-item>
			</swiper>
		</view>
		...
		<view class="input-box" :class="showEmji" @touchmove.stop.prevent="discard">
			...
			<!--#ifdef APP-PLUS-->
			<label for="textMsg" style="margin-top: -12px;">
	...
	export default {
		...
		methods:{			
			init(){					
				...
				uni.connectSocket({
					url: "ws://123.206.113.205:4649/Echo?name=zhangjj",
				...
				uni.onSocketMessage((res) => {
					...
					var content ={content: JSON.parse(res.data).content};//"res.data.content;
					console.log('onMessage1', JSON.parse(res.data).content);
					let msg1 = {id:lastid,uid:1,username:"竞享售后客服",face:"https://www.chinaaos.com/Images/jx128128.png",time:nowDate.getHours()+":"+nowDate.getMinutes(),type:type,msg:content};
					this.screenMsg(msg1);
					console.log('onMessage2', res)
				})
				...
				//   uni.vibrateLong();
				//   //revText(res.data.content,'text');
				//   var nowDate = new Date();
				//   let lastid = this.msgList[this.msgList.length-1].id;
				//   lastid++;
				//   let msg1 ={id:lastid,uid:0,username:"大黑哥",face:"https://www.chinaaos.com/static/img/face.jpg",time:"12:56",type:"text",msg:{content:"为什么温度会相差那么大？"}};
				  
				//   console.log('收到服务器内容：' + res.data);
				  
				// });
				},
			// revText(content,type){
			// var nowDate = new Date();	
			// let lastid = this.msgList[this.msgList.length-1].id;
			// lastid++;
		 //    let msg1 ={id:lastid,uid:0,username:"大黑哥",face:"https://www.chinaaos.com/static/img/face.jpg",time:"12:56",type:"text",msg:{content:"为什么温度会相差那么大？"}};
			// //{id:lastid,uid:1,username:"竞享售后客服",face:"https://www.chinaaos.com/Images/jx128128.png",time:nowDate.getHours()+":"+nowDate.getMinutes(),type:type,msg:content};
			// //this.screenMsg(msg1);
			// this.addTextMsg(msg1);
			// this.$nextTick(function() {
			// 	this.scrollToView = 'msg'+msg1.id
			// });
			// },
			getMsgList(){
				// 消息列表
				...
				let list =[
				    	{id:0,uid:0,username:"我",face:this.Userinfo.HeadPortrait,time:nowDate.getHours()+':'+nowDate.getMinutes(),type:"text",msg:{content:""}}
					]
				// 获取消息中的图片,并处理显示尺寸				
				 this.msgList = list;
				// 滚动到底部
			...
			// 接受消息(筛选处理)
			screenMsg(msg){
				//{"id":7,"uid":0,"username":"大黑哥","face":"https://www.chinaaos.com/static/img/face.jpg","time":"10:31","type":"text","msg":{"content":"<div style=\"display: flex;align-items: center;\">错错错</div>"}} 
			...
			sendText(){
				...
				let newmsg = JSON.stringify({"content":content,"type":"1","name":'',"sender":"zhangjj","receiver":"sa"});
			...
			sendMsg(content,type){
				...
				let lastid = this.msgList[this.msgList.length-1].id;
				lastid++;
				let msg = {id:lastid,uid:0,username:"大黑哥",face:this.Userinfo.HeadPortrait,time:nowDate.getHours()+":"+nowDate.getMinutes(),type:type,msg:content};
				this.screenMsg(msg);
			},
